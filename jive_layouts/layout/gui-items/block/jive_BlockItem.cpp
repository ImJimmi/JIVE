...
private:
    ...
    void testPosition()
    {
...

        {
            ...
            expectEquals(item->getViewport().getX(), 0);
            expectEquals(item->getViewport().getY(), 0);

            tree.setProperty("x", 10.4f, nullptr);
            tree.setProperty("y", 20.89f, nullptr);
            expectEquals(item->getViewport().getX(), 10);
            expectEquals(item->getViewport().getY(), 21);
        }
        {
            juce::ValueTree tree{
                "Component",
            ...
            expectEquals(item->getViewport().getX(), 15);
    ...
    void testSize()
    {
        ...
        expectEquals(item->getWidth(), 10.4f);
        expectEquals(item->getHeight(), 20.89f);
    }
};
