...
//======================================================================================================================
SCENARIO("GUI flex items can shrink if necessary")
{
    GIVEN("a GUI flex container with some children")
    {
        juce::ValueTree tree{
        ...
        WHEN("the first child's flex-shrink is changed")
        {
            childTree1.setProperty("flex-shrink", 2, nullptr);

            AND_WHEN("the container's component is resized")
            {
                item.getComponent().setSize(100, 100);

                THEN("the first child is not as wide as the other children")
                {
                    REQUIRE(item.getChild(0).getComponent().getWidth() < item.getChild(1).getComponent().getWidth());
                }
            }
